...
/** A contiguous stretch of activity, entirely inside one local day. */
export type Block = {
  /** Local calendar day, `YYYY-MM-DD`. */
  day: string;
  /** Epoch milliseconds of the first event in the block. */
  start: number;
  /** Epoch milliseconds of the last event in the block. */
  end: number;
};
...
export function toLocalDay(epochMs: number, timeZone: string): string {
  return dayFormatter(timeZone).format(new Date(epochMs));
}

/** Local wall-clock time for an instant, as `HH:MM`. */
export function toLocalTime(epochMs: number, timeZone: string): string {
...
export function isWorkday(day: string, workdays: readonly number[]): boolean {
  const [year, month, date] = day.split("-").map(Number);
  const index = new Date(Date.UTC(year, month - 1, date)).getUTCDay();
 ...
 */
export function buildBlocks(epochMsList: readonly number[], options: BuildBlocksOptions): Block[] {
...
export function totalSeconds(blocks: readonly Block[]): number {
